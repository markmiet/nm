Shader "Custom/CubismURP"
{
    Properties
    {
        ...
        _TileCount ("Tiles (per axis)",  Range(1,1000)) = 64
        _Jitter ("Jitter (0..1)", Range(0,1)) = 0.35
        _Rotation ("Rotation (radians)", Range(0,6.28318)) = 1.2
        _ColorNoise ("Color Noise", Range(0,1)) = 0.15
        _Seed ("Seed", Float) = 0.0

        _EdgeWidth ("Edge Width", Range(0,5.2)) = 0.02
        _EdgeColor ("Edge Color", Color) = (0,0,0,1)

        _TileSizeVar ("Tile Size Variation", Range(0,1)) = 0.3
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                cellUV = cellUV * scale;
                cellUV = frac(cellUV); // wrap around
                // end tile size variation
                ...
                float edgeFactor = smoothstep(_EdgeWidth, 0.0, edgeDist);

                col.rgb = lerp(_EdgeColor.rgb, col.rgb, edgeFactor);
                col.a *= IN.color.a;
