...
{
    public GameObject[] bonusPrefabit;
    ...
    void FixedUpdate()
    {
        ...
        //IsAnyOfGameObjectVisible jatkaa liikettä
        if (IsGameObjectVisible() || IsAnyOfGameObjectVisible(followers))
        {
            if (!tehty)
            {
                Tee();
    ...
    public bool TarkistaOnkoAmmuttu()
    {
        ...
        foreach (GameObject c in followers)
        {
            ...
            if (c!=null && c.GetComponent<SpriteRenderer>()!=null &&  c.GetComponent<SpriteRenderer>().enabled)
            {
                kaikkiammuttu = false;
                break;
            }
